...
class AdminEditarDropScreen extends StatefulWidget {
  final int dropId;
  const AdminEditarDropScreen({super.key, required this.dropId});

  @override
  State<AdminEditarDropScreen> createState() => _AdminEditarDropScreenState();
}

class _AdminEditarDropScreenState extends State<AdminEditarDropScreen> {
  ...
  Future<void> _cargar() async {
    final drop = await context.read<DropsProvider>().getDetalle(widget.dropId);
    await context.read<ProductosProvider>().cargarProductos();
    if (mounted && drop != null) {
      setState(() {
        _nombreCtrl.text = drop.nombre;
        _imagenActualUrl = drop.imagenUrl;
        _seleccionados.addAll(drop.productos.map((p) => p.id));
        _cargando = false;
      });
    }
  }
  ...
  Future<void> _pickImagen() async {
    ...
    setState(() { _imagenNueva = picked; _previewBytes = bytes; });
  }
  ...
  Future<void> _guardar() async {
    ...
    final ok = await context.read<DropsProvider>().actualizarDrop(
      id: widget.dropId,
          ...
          child: SizedBox(
            ...
            child: Column(
              ...
              children: [
                  ...
                  child: Container(
                    ...
                    child: _previewBytes != null
                        ...
                        : _imagenActualUrl != null
                            ? Image.network(
                                '${ApiService.serverUrl}$_imagenActualUrl',
                                fit: BoxFit.cover,
                ...
                const SizedBox(height: 8),
                Text('Tocá para cambiar la imagen.',
                ...
                const SizedBox(height: 4),
                Text('${_seleccionados.length} producto${_seleccionados.length == 1 ? '' : 's'} seleccionado${_seleccionados.length == 1 ? '' : 's'}.',
                  style: GoogleFonts.dmMono(fontSize: 10, color: AppColors.stone),
                ),
  ...
  Widget _placeholder() => Column(
    ...
    children: [
      ...
      const SizedBox(height: 8),
      Text('Seleccionar imagen', style: GoogleFonts.dmMono(fontSize: 11, color: AppColors.stone)),
    ],
  );
}
